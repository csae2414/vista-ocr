"""``vista-ocr stage 1b`` -- unfrozen OCR-only calibration (Phase I).

Mirrors ``scripts/stage1b_run.py`` for the CLI. Equivalence contract
in ``tests/test_stage_equivalence.py`` (same shape as stage 1/2/3).

Stage 1b bridges the curriculum gap between stage 1 (frozen text-
only) and stage 2 (unfrozen text+layout). Decoder is unfrozen but
the loss is still text-only; gives the decoder a chance to warm up
to the encoder's features without simultaneously learning layout
tokens.
"""
...
def build_parser(*, add_help: bool = True) -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="vista-ocr stage1b",
        description="Stage-1b unfrozen OCR-only calibration.",
        add_help=add_help,
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.init_from and args.init_from.exists():
        ...
        raise SystemExit(
            ...
            "transition stage and requires a stage-1 ckpt."
        )
