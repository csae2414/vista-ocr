"""``vista-ocr stage 3`` -- multitask pretraining (4-task mix).

Mirrors ``scripts/stage3_run.py`` for the CLI. Equivalence contract
in ``tests/test_stage_equivalence.py``.
"""
...
def build_parser(*, add_help: bool = True) -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="vista-ocr stage3",
        description="Stage-3 multitask pretraining (4-task mix).",
        add_help=add_help,
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    def stage3_stream():
        ...
        if len(sources) > 1:
            LOG.info("Phase H/J stage-3 mix: pdfa=%.2f idl=%.2f synth=%.2f",
    ...
    LOG.info("Stage-3 multitask: %d steps, lambda=%.2f, lr=%.2e",
             args.steps, args.lambda_text, args.lr)
    t0 = time.perf_counter()
    history = train(
        model, sample_stream=stage3_stream(), tokenizer=tokenizer,
