"""``vista-ocr stage 2`` -- multimodal pretraining (decoder unfrozen).

Mirrors ``scripts/stage2_run.py`` for the CLI. Equivalence contract
...
def build_parser(*, add_help: bool = True) -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="vista-ocr stage2",
        description="Stage-2 multimodal pretraining (decoder unfrozen).",
        add_help=add_help,
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    history = train(
        model, sample_stream=iter(train_loader), tokenizer=tokenizer,
